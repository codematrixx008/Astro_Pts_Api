...
public sealed class ApiUsageCounterRepository : IApiUsageCounterRepository
{
    ...
    public ApiUsageCounterRepository(IDbConnectionFactory db) => _db = db;
...
WHEN NOT MATCHED THEN
    INSERT (ApiKeyId, DateUtc, RequestCount) VALUES (src.ApiKeyId, src.DateUtc, 1)
OUTPUT inserted.RequestCount;";

        using var conn = _db.Create();
        return await conn.ExecuteScalarAsync<int>(new CommandDefinition(
            sql,
            new { ApiKeyId = apiKeyId, DateUtc = dateUtc.ToDateTime(TimeOnly.MinValue) },
...
WHERE ApiKeyId = @ApiKeyId AND DateUtc = @DateUtc;";
    ...
    public async Task<IReadOnlyList<(DateOnly DateUtc, int RequestCount)>> GetDailyRangeAsync(long apiKeyId, DateOnly from, DateOnly to, CancellationToken ct)
    {
        const string sql = @"
...
ORDER BY DateUtc ASC;";
        ...
        using var conn = _db.Create();
        var rows = await conn.QueryAsync<(DateTime DateUtc, int RequestCount)>(new CommandDefinition(
            sql,
            new
            {
                ApiKeyId = apiKeyId,
        ...
        return rows.Select(r => (DateOnly.FromDateTime(r.DateUtc), r.RequestCount)).ToList();
    }
}
